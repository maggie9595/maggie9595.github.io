...
$(document).ready(function() {
    ...
    $.fn.extend({ fifteen:
        ...
        function(square_size) {
            ...
            // it is the selector - "#slider_object" from  $("#slider_object").fifteen(175);
            var gameObjectElement = '#' + $(this).attr('id'); 

            var sqSize = square_size + 'px';
            var boardSize = (square_size * 4) + 'px';

            // Inject DIV into slider_object, this is our game board
            $(gameObjectElement).html('<div id="slider-board"></div>'); 

            $('#slider-board').css({ position:'absolute', width: boardSize, height: boardSize, border: '1px solid gray' });
            ...
            for (var i = 0; i < 16; i++) {
                ...
                $('#slider-board').append("<div style='left: " + ((i % 4) * square_size) + "px; top: " + Math.floor(i / 4) * square_size + "px; width: " + square_size + "px; height: " + square_size + "px; background-position: " + (-(i % 4) * square_size) + "px " + -Math.floor(i / 4) * square_size + "px '></div>");
            }

            // Empty up the 16th square, as the starting point
            // EmptySquare = 16
            $('#slider-board').children("div:nth-child(" + EmptySquare + ")").css({backgroundImage: "", background: "#ffffff"});

            // Attach click event to each square
            $('#slider-board').children('div').click(function() {
    ...
    function Move(clicked_square, square_size) {
        ...
        var oldx = $('#slider-board').children("div:nth-child(" + EmptySquare + ")").css('left');
        var oldy = $('#slider-board').children("div:nth-child(" + EmptySquare + ")").css('top');
        ...
        if (movable) {
            ...
            $(clicked_square).animate({ left: oldx, top: oldy }, 200, function() {
                ...
                $('#slider-board').children("div:nth-child(" + EmptySquare + ")").css('left', newx);
                $('#slider-board').children("div:nth-child(" + EmptySquare + ")").css('top', newy);
            });
        }
    }

    // Ok, we're ready to initialize the game, let's do it.
    // Create a game with 175 by 175 squares inside "#slider_object" div:
    $('#slider_object').fifteen(175);
});
